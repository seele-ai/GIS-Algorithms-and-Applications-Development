...
namespace GIS
{
    ...
    public class ProjUTM : ProjectionCS
    {
        ...
        /// <param name="latitudeBand">
        /// 纬度行字母（C~X，不含 I/O）。UTM 的四边形按“带号+行字母”标记（如北京为 50S），
        /// 传入后用它作为坐标系统名称中的后缀；不传时退化为用半球字母 N/S。
        /// </param>
        public ProjUTM(Int32 zone, bool isNorthernHemisphere = true, Ellipsoid ellipsoid = null,
            char latitudeBand = '\0')
            : base(MakeName(zone, isNorthernHemisphere, latitudeBand), ellipsoid ?? Ellipsoids.WGS84, ProjectionTypeConstant.UTM)
        {
            OriginLatitude = 0;
        ...
        //自动命名：优先使用纬度行字母（UTM 四边形的标准标记），否则用半球
        private static string MakeName(Int32 zone, bool isNorthernHemisphere, char latitudeBand)
        {
            string suffix = latitudeBand == '\0'
                ? (isNorthernHemisphere ? "N" : "S")
                : char.ToUpperInvariant(latitudeBand).ToString();
            return "WGS_1984 UTM Zone " + zone + suffix;
        }
