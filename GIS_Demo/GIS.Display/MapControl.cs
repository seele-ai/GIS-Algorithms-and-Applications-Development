...
namespace GIS.Display
{
    public class MapControl : UserControl, IMapControl
    {
        ...
        public void SetExtent(Envelope extent)
        {
            ...
            // 注意：这里不能用固定值（如10）兜底——经纬度坐标的量级只有零点几度，会被放大成整度范围。
            double minimum = DegenerateFloor(extent);
        ...
        public void FullExtent()
        {
            ...
            double padding = Math.Max(Math.Max(extent.Width, extent.Height) * 0.05, DegenerateFloor(extent));
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // 注记绘制在所有图层几何之上
            if (renderer is ILayerLabelRenderer labelRenderer)
            {
                foreach (Layer layer in layers.Where(l => l.Visible))
                {
                    var state = e.Graphics.Save();
                    try { labelRenderer.DrawLayerLabels(e.Graphics, layer, transform, ClientRectangle); }
                    finally { e.Graphics.Restore(state); }
                }
            }
            // 高亮叠加在所有普通图层之上。
        ...
        // 退化范围（单点/水平线/垂直线）的最小跨度：按坐标量级取相对值，避免经纬度被放大
        private static double DegenerateFloor(Envelope extent)
        {
            double magnitude = Math.Max(Math.Abs(extent.CenterX), Math.Abs(extent.CenterY));
            double floor = magnitude > 0 ? magnitude * 1e-4 : 1e-6;
            double span = Math.Max(extent.Width, extent.Height);
            return span > floor ? 0 : floor;   // 正常范围不需要兜底
        }
    }
}
