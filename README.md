...
| 模块2 图形显示与图层管理 | 白宇丹 | 地图绘制、图层管理、鼠标选择 | 已完成（177 项自动检查通过） |
| 模块3 图层渲染与编辑 | 赵佶昊 | 点/线/面符号编辑器、唯一值/分级渲染、注记、色带、分带选择、渲染符号存取、坐标系统一到 WGS84、GCJ-02、自定义线段特性 | 已完成（177 项自动检查通过） |
...
└── GIS_Demo/
    ...
    ├── GIS.Display.Checks/     自动检查程序（177 项检查）
    └── README.md
```
...
# 模块2 演示程序（图形界面，启动后自动显示 6 个样例图层、12 个要素）
...
- 唯一值/分级渲染的**图例**在图层面板中逐项展开（符号预览 + 值/区间标签）；简单渲染只显示一个符号。图例上方的标题就是**绑定字段名**（`HeadTitle`，`ShowHead=false` 时不显示）：`Field` 的 setter 会让标题自动跟随绑定字段（标题为空、或仍等于原字段名时同步更新，**只有显式改过标题才不再跟随**），因此读取已有渲染符号文件后重新绑定字段，面板里显示的字段会立即更新，不会再出现“显示的绑定字段固定、无法更改”。
...
可用的 7 种形状（编辑器下拉框与渲染符号文件中的“形状=”取值同名）：

| 形状 | 枚举值 | 典型用途 | 绘制规则 |
|---|---|---|---|
| 圆形 / 方形 / 三角形 / 十字 | `Circle` / `Square` / `Triangle` / `Cross` | 一般点位 | 填充 = `Color`，边框 = `OutlineColor` + `OutlineWidth` |
| **五角星** | `Star` | 首都等重要点位 | 10 个顶点（外接圆半径 = `Size`/2、凹角半径 = 0.382×外接半径）、一个角朝上；填充与边框规则同上 |
| **实心点圆环** | `SolidDotCircle` | 省会等次级点位 | 外围空心圈 + 中心实心点 |
| **空心点圆环** | `HollowDotCircle` | 普通城市等一般点位 | 外围空心圆 + 中心空心点 |
| （感叹号） | `Exclamation` | 仅“绑定属性错误”提示 | 只显示在图例中，不绘制到地图 |

两种“圆环”符号的约定：**外圈直径 = `Size`、中心点直径 ≈ `Size`/3**；圈与中心同色（取 `Color`，`Color` 为透明时取 `OutlineColor`）；线宽按 `OutlineWidth` 与尺寸等比换算并保证最小 0.8 像素可见。演示程序里新增了“城市（典型点位符号）”图层（唯一值渲染）：首都=五角星、省会=实心点圆环、普通城市=空心点圆环。
...
- **点/多点**：候选锚点依次为符号的**右上 → 右下 → 左上 → 左下**（都整体落在符号外侧，让开「符号半径 + 3 像素」）；
- **线/复合折线**：以折线中点（`GetMidPoint`）为中心居中摆放，并允许整体上/下微调；
- **面/复合面**：以**外包矩形中心**（`Envelope.Center`）为中心居中摆放，同样允许微调；
- **摆放与判交都用注记实际占据的平行四边形**（`LabelPlacer.Quad` 给出 4 个角点，而不是外接矩形）：
  - **摆放**：取注记平行四边形的 4 个角中“朝向该方位、离要素最近的那个角”（右上取 `x−y` 最小、右下取 `x+y` 最小、左上取 `x+y` 最大、左下取 `x−y` 最大），把它放到与要素相距「符号半径 + 3 像素」的位置上。因此 **`RotateAngle` 取任意角度时，注记离要素最近的那个角总是紧贴要素外侧**（不会像按外接矩形摆放那样在旋转后忽远忽近；角度为 0 时与不旋转的摆放完全一致，原有布局不变）。线/面则是让平行四边形（与外接矩形同心）的中心对准定位点；
  - **判交**：两条注记按各自的平行四边形用分离轴定理（SAT）判断是否真的相交，分离时再算形状间最短距离，小于 2 像素才算冲突——外接矩形相交但实际形状不冲突时**不再误报**（此前是把外接矩形再按角度旋转一次，形状本身是错的）；
...
图例里的符号可以直接单击打开对应的符号编辑器（`LayerControl.CanEditSymbolDirectly` 判定）；**只有“绑定属性错误”状态例外**——此时画的是专用的红色感叹号符号（形状不在点符号编辑器的四种形状里），单独设置它也没有意义，因此**不响应单击、光标也不再是手型**，行内与符号上都挂着图层右键菜单：从右键菜单的 **“修改图层符号/渲染”** 进入渲染设置、重新绑定字段后生成即可恢复。符号编辑器本身也加了容错（形状下拉框按合法范围钳制），即使直接打开错误符号也不会再抛 `ArgumentOutOfRangeException`；渲染设置窗口里的符号列表同样遵循这一规则。
