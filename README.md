...
| 模块2 图形显示与图层管理 | 白宇丹 | 地图绘制、图层管理、鼠标选择 | 已完成（174 项自动检查通过） |
| 模块3 图层渲染与编辑 | 赵佶昊 | 点/线/面符号编辑器、唯一值/分级渲染、注记、色带、分带选择、渲染符号存取、坐标系统一到 WGS84、GCJ-02、自定义线段特性 | 已完成（174 项自动检查通过） |
...
└── GIS_Demo/
    ...
    ├── GIS.Display.Checks/     自动检查程序（174 项检查）
    └── README.md
```
...
- **设置旋转角后按方位反推锚点**：候选位置不是用“未旋转的宽高”算的，而是先把注记矩形绕锚点转到目标角度、取它的**旋转后外接矩形**，再让这个转好之后的矩形贴到要素外侧（线/面则是让旋转后的矩形中心对准定位点）。因此 **`RotateAngle` 取任意角度时，四个方位的注记都与要素保持同样的间隙**——此前按未旋转宽高摆放，一旦设置旋转角，左上/左下等方位的注记会被“转出去”离要素很远（自检覆盖 0°/30°/45°/90°/−45°/135° 四个方位的贴合，以及旋转 90° 的实际绘制间隙）；
